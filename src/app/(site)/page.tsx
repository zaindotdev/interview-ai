...
import {HowItWorks, KeyFeaturesData} from "@/lib/types";
...
const Landing = ()=>{
  return <Container>
    ...
    <section id={"home"} className={"w-full min-h-screen flex items-center justify-center"}>
      <HeadingContainer>
        ...
        >
          Ace you next interview with AI powered precision.
        </motion.h1>
        <motion.p
            initial={{opacity: 0, y: 20}}
            animate={{opacity: 1, y: 0}}
  ...
  <section id={'how-it-works'} className={"w-full py-24"}>
    <HeadingContainer>
      <motion.h2 initial={{opacity:0, y:20}} whileInView={{opacity:1, y:0}} transition={{duration:0.8, ease:"linear"}} className={"text-center text-orange-600 font-medium text-base/8 md:text-lg/8"}>
        How it works
      </motion.h2>
      <motion.h1 initial={{opacity: 0, y: 20}} whileInView={{opacity: 1, y: 0}} transition={{duration: 0.8, delay: 0.2, ease: "linear"}} className={"text-2xl/8 md:text-5xl text-center font-semibold mt-4"}>
        Master Your Interview Skills with Our AI-Powered Platform
      </motion.h1>
      <motion.p initial={{opacity: 0, y: 20}} whileInView={{opacity: 1, y: 0}} transition={{duration: 0.8, delay: 0.4, ease: "linear"}} className={"mt-6 text-base md:text-xl/8 text-center font-medium text-gray-600"}>
    ...
    <div className={"w-full mt-8"}>
      <div className={"w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4"}>
        {howItWorksData.map((data, index) => (
            <motion.div key={`how-it-works-${index}`} initial={{opacity:0, y:20}} whileInView={{opacity:1, y:0}} transition={{duration:0.8, delay:index*0.5, ease:"linear"}} className={"w-full"}>
              <Card  className={"max-w-xl bg-inherit"}>
                <CardHeader className={"flex items-center justify-center gap-4"}>
                  <motion.div initial={{opacity:0, y:20}} whileInView={{opacity:1, y:0}} transition={{duration:0.8, delay:0.2, ease:"linear"}} className={'size-24 shrink-0 rounded-full bg-gradient-to-br from-orange-500 to-orange-700 flex items-center justify-center'}>
                    <h1 className={"text-white text-2xl/8 font-bold"}>
                      {data.index}
                    </h1>
                  </motion.div>
                  <CardTitle>
                    <motion.h1 initial={{opacity:0,y:20}} whileInView={{opacity:1,y:0}} transition={{ duration: 0.8,delay:0.2, ease: "easeInOut" }} className={"text-xl md:text-2xl/8 font-bold text-orange-500"}>
                      {data.title}
                    </motion.h1>
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <motion.p initial={{opacity:0, y:20}} whileInView={{opacity:1, y:0}} transition={{duration:0.8, delay:0.2, ease:"linear"}} className={"max-w-md mx-auto text-center text-base md:text-lg/8 text-gray-600"}>{data.description}</motion.p>
                </CardContent>
              </Card>
            </motion.div>
        ))}
    ...
    <section className={"w-full py-24"} id={"demo"}>
      ...
      <motion.div initial={{opacity: 0, y: 20}} whileInView={{opacity: 1, y: 0}} transition={{duration: 0.8, delay: 0.4, ease: "linear"}} className={"mt-8 rounded-xl overflow-hidden max-w-4xl mx-auto"}>
      ...
      <div className={"w-full my-8"}>
          <ul className={"grid grid-cols-1 md:grid-cols-2 gap-8"}>
            {keyFeaturesData.map((data, index) => (
                <motion.li key={`key-features-${index}`} initial={{opacity:0, y:20}} whileInView={{opacity:1, y:0}} transition={{duration:0.8, delay:index*0.2  , ease:"linear"}} className={"flex items-center gap-4 md:border-b-0 border-b-2 p-2"}>
                  ...
                  <div className={"flex-1"}>
                    <motion.h2 initial={{opacity:0,y:20}} whileInView={{opacity:1,y:0}} transition={{duration:0.8, delay:0.4,ease:"linear"}} className={"text-orange-500 font-bold text-xl/8 text-pretty"}>
                      {data.title}
                    </motion.h2>
                    <motion.p initial={{opacity:0,y:20}} whileInView={{opacity:1,y:0}} transition={{duration:0.8, delay:0.6,ease:"linear"}} className={"mt-1 text-gray-600 text-base"}>
                      {data.description}
                    </motion.p>
                  </div>
                </motion.li>
